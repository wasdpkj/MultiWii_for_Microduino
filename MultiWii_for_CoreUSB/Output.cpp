...
// its not possible to change a PWM output pin just by changing the order
  ...
  uint8_t PWM_PIN[8] = {9,10,11,3,6,5,A2,12};   //for a quad+: rear,right,left,front
      ...
      uint8_t PWM_PIN[8] = {6,9,8,5,7,4,SW_PWM_P3,SW_PWM_P4};   //
    #else
    uint8_t PWM_PIN[8] = {9,8,7,6,10,A1,SW_PWM_P3,SW_PWM_P4};   //for a quad+: rear,right,left,front
    #endif
  #else
    #if defined(TEENSY20)
      ...
      uint8_t PWM_PIN[8] = {6,9,8,5,7,4,SW_PWM_P3,SW_PWM_P4};   //
    #else
      uint8_t PWM_PIN[8] = {9,8,7,6,5,4,SW_PWM_P3,SW_PWM_P4};   //for a quad+: rear,right,left,front
    #endif
  #endif
#endif
#if defined(MEGA)
...
void writeMotors() { // [1000;2000] => [125;250]
  ...
  /******** Specific PWM Timers & Registers for the atmega32u4 (Promicro)   ************/
    ...
    Temp2 = motor[3] - 1000;
        ...
        #ifndef EXT_MOTOR_RANGE
          TC4H = motor[0]>>8; OCR4A = (motor[0]&0xFF); //  pin 6
        #else
          TC4H = (((motor[0]-1000)<<1)+16)>>8; OCR4A = ((((motor[0]-1000)<<1)+16)&0xFF); //  pin 6
        #endif
      #else
      ...
      #elif defined(EXT_MOTOR_16KHZ)
        OCR1B = motor[1] - 1000;        //  pin 8
      #elif defined(EXT_MOTOR_8KHZ)
        OCR1B = (motor[1]-1000) << 1;   //  pin 8
      #else
        OCR1B = motor[1]<<3; //  pin 8
      #endif
    #endif
    #if (NUMBER_MOTOR > 2) // Timer 4 A & D [1000:2000] => [1000:2000]
        #ifdef EXT_MOTOR_RANGE 
          OCR1C = ((motor[2]<<4) - 16000) + 128;
        #elif defined(EXT_MOTOR_64KHZ)
          OCR1C = (motor[2] - 1000) >> 2;
        #elif defined(EXT_MOTOR_32KHZ)
          OCR1C = (motor[2] - 1000) >> 1;
        #elif defined(EXT_MOTOR_16KHZ)
          OCR1C = motor[2] - 1000;        //  pin 7
        #elif defined(EXT_MOTOR_8KHZ)
          OCR1C = (motor[2]-1000) << 1;   //  pin 7
        #else
          OCR1C = motor[2]<<3; //  pin 7
        #endif
    #endif
    #if (NUMBER_MOTOR > 3)
      #ifdef EXT_MOTOR_RANGE 
        TC4H = (((motor[3]-1000)<<1)+16)>>8; OCR4A = ((((motor[3]-1000)<<1)+16)&0xFF); //  pin 6
      #elif defined(EXT_MOTOR_64KHZ)
        Temp2 = Temp2 >> 2;
        TC4H = Temp2 >> 8;
        OCR4A = Temp2 & 0xFF;           //  pin 6
      #elif defined(EXT_MOTOR_32KHZ)
        Temp2 = Temp2 >> 1;
        TC4H = Temp2 >> 8;
        OCR4A = Temp2 & 0xFF;           //  pin 6
        ...
        OCR4A = Temp2 & 0xFF;           //  pin 6
      #else
        TC4H = motor[3]>>8; OCR4A = (motor[3]&0xFF); //  pin 6
      #endif
    #endif    
    #if (NUMBER_MOTOR > 4)
      #if !defined(HWPWM6)
      ...
      #else
        #ifndef EXT_MOTOR_RANGE 
          OCR3A = motor[4]<<3; //  pin 5
          TC4H = motor[5]>>8; OCR4D = (motor[5]&0xFF); //  pin 4
        #else
          OCR3A = ((motor[4]<<4) - 16000) + 128;
          TC4H = (((motor[5]-1000)<<1)+16)>>8; OCR4D = ((((motor[5]-1000)<<1)+16)&0xFF); //  pin 13       
        #endif  
      #endif
    #endif
    #if (NUMBER_MOTOR > 6)
...
//    TCCR3A = (1<<WGM31);    //TCCR1C = (1<<WGM11);
      ...
      TCCR4D = 0;
      TCCR4C |= (1<<COM4A1)|(1<<PWM4A); // connect pin 6 to timer 4 channel A
    #else
      ...
      #endif
      #if (NUMBER_MOTOR > 2)
        TCCR1A |= _BV(COM1C1); // connect pin 7 to timer 1 channel C  //pkj
      #endif
        ...
        #endif
        TCCR4A |= (1<<COM4A1)|(1<<PWM4A); // connect pin 6 to timer 4 channel D
      #endif
      #if (NUMBER_MOTOR > 4)
        #if defined(HWPWM6) 
          ...
          TCCR4C |= (1<<COM4D1)|(1<<PWM4D); // connect pin 4 to timer 4 channel D
        #else
          initializeSoftPWM();
        #endif
      #endif
